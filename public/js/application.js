...
(function(exports) {
  ...
  exports.init = function() {
    ...
    $('.navbar-search').submit(function(e) {
        ...
        $('.container .tweets').fadeOut(0, function() {
          ...
          $('.tweets-header-title').empty().text(templates.searching_for.render({
            'query': searchInput
          }));
          $(this).empty();
          $(this).fadeIn(100);
              ...
              if (month_bar.length < 12) {
                for (var m = 0; m < 12; m++) {
                  if (!month_bar[m] || month_bar[m].match(/class="value">(\d+)<\/span>/)[1] != m + 1) {
                    month_bar.splice(m, 0, templates.empty_month.render({
                      this_month: m + 1
                    }));
                  }
                }
          ...
          if (readyCount == statusArr.length) {
            ...
            $('.container .tweets').fadeOut(100, function() {
              ...
              _.each($('.e-content .p-name, .h-card .screen-name'), function(tweet) {
                _.each($(tweet).find("*").andSelf(), function(e) {
                  $(e).highlight(escapeRegexCharacters(searchString), "found");
                });
              });
              $(this).fadeIn(100);
            });
  ...
  exports.loadScript = function(tweet_month, callback) {
    ...
    console.log(tweet_month['file_name']);
